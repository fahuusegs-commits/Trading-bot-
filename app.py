...
try:
    ...
    if not data.empty:
        ...
        # Condition 3: MACD
        macd_col = [col for col in data.columns if 'MACD_' in col and 's' not in col][0]
        signal_col = [col for col in data.columns if 'MACDs_' in col][0]
        
        if last_row[macd_col] > last_row[signal_col]: buy_signal += 1
        else: sell_signal += 1
        ...
        # Bollinger Bands
        bb_upper = [col for col in data.columns if 'BBU_' in col][0]
        bb_lower = [col for col in data.columns if 'BBL_' in col][0]
        fig.add_trace(go.Scatter(x=data.index, y=data[bb_upper], line=dict(color='gray', width=1), name="BB Upper"))
        fig.add_trace(go.Scatter(x=data.index, y=data[bb_lower], line=dict(color='gray', width=1), name="BB Lower"))
